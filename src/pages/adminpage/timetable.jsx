...
function TimetableAdmin() {
  ...
  useEffect(() => {
    fetch("http://localhost:8080/api/timetable/hours")
      .then(r => r.json())
      .then(d => { if (Array.isArray(d)) setGymHours(d); })
      .catch(() => {});

    fetch("http://localhost:8080/api/timetable/weekly")
      .then(r => r.json())
      .then(d => { if (Array.isArray(d)) setTimetable(d); })
      .catch(() => {});
  }, []);
  ...
  async function handleSave() {
    ...
    try {
      const [r1, r2] = await Promise.all([
        fetch("http://localhost:8080/api/timetable/hours",  { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(gymHours) }),
        fetch("http://localhost:8080/api/timetable/weekly", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(timetable) }),
      ]);
                ...
                {timetable.map((row, i) => (
                  <tr key={i} className={i % 2 === 0 ? "bg-white/5" : "bg-transparent"}>
                    ...
                    <td className="px-2 py-2 text-center">
                      <button
                        onClick={() => setTimetable(timetable.filter((_, idx) => idx !== i))}
